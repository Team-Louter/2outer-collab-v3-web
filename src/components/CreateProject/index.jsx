...
export default function CreateProject({ setCreateProject, onClose, onProjectCreated }) {
  ...
  // 닫기 함수 처리 (setCreateProject 또는 onClose 지원)
  const handleClose = () => {
    if (onClose) {
      onClose();
    } else if (setCreateProject) {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      toast.error('프로젝트 생성 중 오류가 발생했습니다', toastcode(3000));
      toast.clearWaitingQueue();
    }
  };
      ...
      <div className={styles.modalBackground}>
        <div className={styles.modalContent}>
          ...
          <input 
            ...
            accept="image/*"
          />
          <button className={styles.create} onClick={handleCreate}>생성하기</button>
        </div>
      </div>
      {showCrop && 
        <CropImage 
